...
module.exports = {
	...
	async execute(interaction, client) {
		...
		*/
		...
		const collector = mesg.createMessageComponentCollector({ componentType: ComponentType.SelectMenu, time: 15000 });
		...
		async function newFunction(collect) {
			...
			collector.on('collect', async (i) => {
				if (i.customId === 'basicPerms') {
					...
					const msg = await i.reply({ embeds: [roleCreateEmbed], components: [permsSelectRow1, permsSelectRow2, permsSelectRow3, completeButtonRow] });
					return msg;
				}
				else if (i.customId === 'voicePerms') {
					i.values.forEach(string2 => {
