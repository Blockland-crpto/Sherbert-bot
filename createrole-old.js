	...
	async execute(interaction, client) {
		...
		collector.on('collect', async i => {
			...
			collector2.on('collect', async im => {
				...
				const collector3 = message3.createMessageComponentCollector({ componentType: ComponentType.SelectMenu, time: 15000 });
				collector3.on('collect', async imm => {
					imm.values.forEach(string3 => {
					...
					await i.followUp({ embeds: [roleCompletedEmbed], components: [], ephemeral: true });
